...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:variable name="mainH2Nodes" select="$h2Para[position()>number($abstractNodeH2Index) and number($referenceNodeH2Index)>position()]"/>
          ...
          <div class="block" id="Test-ImgSrc">
            ...
            <div class="FulltextWrapper">
              ...
              <section xmlns="" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:meta="http://www.springer.com/app/meta" class="Section1 RenderAsSection1" id="CopyrightMessage">
                ...
                <div class="js-CollapseSection">
                  <div xmlns="http://www.w3.org/1999/xhtml" class="ArticleCopyright">
                © The Author(s).<xsl:value-of select="$publishedYear" disable-output-escaping="yes"/>
              </div>
                </div>
              </section>
            </div>
                ...
                <nav xmlns="" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:meta="http://www.springer.com/app/meta" id="ArticleNavigation" class="table-of-contents" aria-label="table of contents">
                  <div class="table-of-contents__content" data-sticky="toc">
                    ...
                    <ul class="table-of-contents__menu is-open" role="menu" id="tocMenu">
                      ...
                      <xsl:apply-templates select="/w:body/w:p[w:pPr/w:pStyle[@w:val='2']]" mode="Daohang"/>
  ...
  <!--**************************************************导航**************************************************-->
  <xsl:template match="/w:body/w:p[w:pPr/w:pStyle[@w:val='2']]" mode="Daohang">
    <xsl:choose>
      <xsl:when test="w:r[w:t='Abstract']">
      ...
      <xsl:when test=". is $referenceNode">
        <li role="menuitem" class="table-of-contents__item">
          <a class="table-of-contents__link" href="#Bib1" data-event-category="Table of Contents" data-track-event="click" data-event-action="Link Clicked" data-event-label="Abstract">
            <xsl:value-of select="."/>
          </a>
        </li>
      </xsl:when>
      <xsl:otherwise>
        ...
        <xsl:variable name="H2IndexInH2P">
          <xsl:call-template name="getNodeIndex">
            <xsl:with-param name="nodes" select="$h2Para"/>
            <xsl:with-param name="node" select="."/>
          </xsl:call-template>
        </xsl:variable>
        <!--获取H2节点 在所有的 H2 H3节点中的索引-->
        <xsl:variable name="H2IndexInH2AndH3">
          <xsl:choose>
            <!--这是因为Abstract是第1个H2节点,所以第2个H2节点是Abstract之后的第1个H2节点-->
            <xsl:when test="number($H2IndexInH2P)=2">
              <xsl:value-of select="1"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:variable name="without1stH2_H2IndexInH2AndH3">
                <xsl:call-template name="getNodeIndex">
                  <xsl:with-param name="nodes" select="$H3AndH2After2thH2"/>
                  <xsl:with-param name="node" select="."/>
                </xsl:call-template>
              </xsl:variable>
              <xsl:value-of select="number($without1stH2_H2IndexInH2AndH3)+1"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>

        <li role="menuitem" class="table-of-contents__item">
          <a class="table-of-contents__link" href="{concat('#Sec',number($H2IndexInH2AndH3))}" data-event-category="Table of Contents" data-track-event="click" data-event-action="Link Clicked" data-event-label="Abstract">
            <xsl:value-of select="."/>
          </a>
        </li>
      </xsl:otherwise>
  ...
  <xsl:template name="abstractPart">
    ...
    <!--获取Abstract节点的下一个 H2节点 在全文所有非空段落节点中的位置-->
    <xsl:variable name="resultNodeEndIndex">
      <xsl:call-template name="getNodeIndex">
        <xsl:with-param name="nodes" select="$normalPara"/>
    ...
    <!--获取摘要节点的下一个H3节点-->
    <xsl:variable name="firstH3AfterAbstract" select="$abstractNode/following-sibling::w:p[w:pPr/w:pStyle[@w:val='3']][1]"/>
  ...
  <xsl:template name="mainPart">
    <xsl:for-each select="$mainH2Nodes">
      ...
      <section xmlns="" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:meta="http://www.springer.com/app/meta" id="{concat('Sec',number($H2IndexInH2AndH3))}" class="Section1 RenderAsSection1">
  ...
  <xsl:template name="paragraphWithoutH3BeforeHtml">
    <xsl:param name="startIndex"/>
    <xsl:param name="endIndex"/>
    <xsl:for-each select="$normalPara[position() &gt;number($startIndex) and position() &lt;number($endIndex)]">
  ...
  <xsl:template name="comPara_instrTextNull_Clear">
    ...
    <xsl:for-each select="$comPara/w:r">
      ...
      <xsl:variable name="IsInSequence">
        <xsl:call-template name="nodeInSequence">
          <xsl:with-param name="sequence" select="$comPara_instrTextNull"/>
          <xsl:with-param name="node" select="."/>
        </xsl:call-template>
      </xsl:variable>
      <xsl:if test="not($IsInSequence)">
        <xsl:copy-of select="."/>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>

  <xsl:template name="nodeInSequence">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:for-each select="1 to count($wr_instrText_Ref)">
          ...
          <xsl:choose>
            <!--当是第一个引用的时候，拿到从开头到这个引用位置的内容直接输出-->
          ...
          <xsl:variable name="pure_Ref_Content" select="$wr_instrText_Ref[$index]"/>
          <xsl:variable name="ref_position">
            <xsl:call-template name="getNodeIndex_for_Ref">
              <xsl:with-param name="nodes" select="$referencedata"/>
              <xsl:with-param name="node" select="$pure_Ref_Content"/>
            </xsl:call-template>
          </xsl:variable>
          <span class="CitationRef">
            <a href="{concat('#CR',$referencedata[number($ref_position)]/@w:id)}">
  ...
  <xsl:template name="getNodeIndex_for_Ref">
    <xsl:param name="nodes"/>
    <xsl:param name="node"/>
    <xsl:for-each select="$nodes">
      <xsl:choose>
        <xsl:when test="contains(string($node),string(@w:name))">
          <xsl:value-of select="position()"/>
        </xsl:when>
      </xsl:choose>
    </xsl:for-each>
    ...
    <xsl:variable name="result">
      <xsl:for-each select="$sequence">
        <xsl:if test="contains(string(node),string(./@w:name))">
          <xsl:value-of select="position()"/>
        </xsl:if>
      </xsl:for-each>
    </xsl:variable>
    <xsl:value-of select="max($result)"/>-->
    <!--因为经过上面的循环之后 形成了一个序列 有几个是空 有1个是有真正位置的值 这里可能出错，但是理论上应该不会有两个书签同一个id的，也可以把sum改成max-->
