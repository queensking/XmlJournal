...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:template name="commonParaOutPut">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:for-each select="$wr_instrText">
          <xsl:choose>
            <xsl:when test="starts-with(normalize-space(.),'REF')">
              ...
              <xsl:variable name="newCommonParaOutPut">
                <xsl:copy-of select="$comPara/w:r[position()>number($wr_fldCharBegin_Index)]"/>
                <xsl:value-of select="$wr_ref_content"/>
                <xsl:copy-of select="$comPara/w:r[number($wr_fldCharEnd_Index)>position()]"/>
              </xsl:variable>
              <!--<xsl:call-template name="commonParaOutPut">
            ...
            <xsl:when test="normalize-space(.)=''">
              ...
              <xsl:variable name="newCommonParaOutPut">
                <xsl:copy-of select="$comPara/w:r[position()!=$instrTextIsNull_Index]"/>
              </xsl:variable>
