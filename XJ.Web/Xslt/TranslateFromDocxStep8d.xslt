...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <!--<xsl:template name="newCommonParaOutPut">
    <xsl:param name="para"/>
    <xsl:param name="replacePart"/>
    <xsl:param name="startIndex"/>
    <xsl:param name="endIndex"/>
    -->
  <!--将上面的点复制-->
  <!--
    <xsl:for-each select="$para/w:r[position()>number($startIndex)]">
      <xsl:copy-of select="."/>
    </xsl:for-each>
    <xsl:value-of select="$replacePart"/>

    <xsl:for-each select="$para/w:r[number($endIndex)>position()]">
      <xsl:copy-of select="."/>
    </xsl:for-each>
  </xsl:template>-->
  ...
  <xsl:template name="commonParaOutPut">
    ...
    <!--获得所有 有w:instrText的 w:r-->
    <xsl:variable name="wr_instrText" select="$comPara/w:r[w:instrText]"/>

    <xsl:choose>
      <xsl:when test="count($wr_instrText)=0">
        <xsl:value-of select="."/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="starts-with(normalize-space($wr_instrText[1]),'REF')">
            <xsl:variable name="wr_fldCharBegin" select="$wr_instrText[1]/preceding-sibling::w:r[w:fldChar/@w:fldCharType='begin'][1]"/>
            <xsl:variable name="wr_fldCharSeparate" select="$wr_instrText[1]/following-sibling::w:r[w:fldChar/@w:fldCharType='separate'][1]"/>
            <xsl:variable name="wr_fldCharEnd" select="$wr_instrText[1]/following-sibling::w:r[w:fldChar/@w:fldCharType='end'][1]"/>
            ...
            <xsl:variable name="wr_ref_content" select="$comPara/w:r[position()>number($wr_fldCharSeparate_Index) and number($wr_fldCharEnd_Index)>position()]"/>
            
            <!--我是<xsl:value-of select="$wr_ref_content"/>漫哥-->

            <!--<xsl:call-template name="newCommonParaOutPut">
                <xsl:with-param name="startIndex" select="$wr_fldCharBegin_Index"/>
                <xsl:with-param name ="endIndex" select="$wr_fldCharEnd_Index"/>
                <xsl:with-param name="replacePart" select="$wr_ref_content"/>
                <xsl:with-param name="para" select=""
              </xsl:call-template>-->

            <xsl:variable name="newCommonParaOutPut">
              <xsl:copy-of select="$comPara/w:r[number($wr_fldCharBegin_Index)>position()]"/>
              <xsl:copy>
                <w:r>
                  <w:t>
                    我真的是你们的爸爸<xsl:value-of select="$wr_ref_content"/>我真的是你们的爸爸
                  </w:t>
                </w:r>
              </xsl:copy>
              <xsl:copy-of select="$comPara/w:r[position()>number($wr_fldCharEnd_Index)]"/>
            </xsl:variable>

            <!--<xsl:value-of select="$newCommonParaOutPut"/>-->
            <xsl:call-template name="commonParaOutPut">
              <xsl:with-param name="comPara" select="$newCommonParaOutPut"/>
            </xsl:call-template>
          </xsl:when>
          <xsl:when test="normalize-space($wr_instrText[1])=''">
            <xsl:variable name="instrTextIsNull_Index">
              <xsl:call-template name="getNodeIndex">
                <xsl:with-param name="nodes" select="$comPara/w:r"/>
                <xsl:with-param name="node" select="$wr_instrText[1]"/>
              </xsl:call-template>
            </xsl:variable>

            <xsl:variable name="newCommonParaOutPut">
              <xsl:copy-of select="$comPara/w:r[position()!=number($instrTextIsNull_Index)]"/>
            </xsl:variable>
    ...
    <!--<xsl:for-each select="$comPara/w:r">
      -->
    <!--获取当前w:r节点下面的w:instrText节点-->
    <!--
      <xsl:variable name="instrText" select="w:instrText"/>
      <xsl:choose>
        -->
    <!--如果当前w:r节点存在w:instrText节点的时候-->
    <!--
        <xsl:when test="$instrText">
          <xsl:choose>
            -->
    <!--当是引用节点的时候-->
    <!--
            <xsl:when test="starts-with(normalize-space($instrText),'REF')">
              -->
    <!--preceding-sibling是逆序的position=1代表跟他靠的最近的那个节点-->
    <!--
              <xsl:variable name="wr_fldCharBegin" select="preceding-sibling::w:r[w:fldChar/@w:fldCharType='begin'][1]"/>
              <xsl:variable name="wr_fldCharSeparate" select="following-sibling::w:r[w:fldChar/@w:fldCharType='separate'][1]"/>
              <xsl:variable name="wr_fldCharEnd" select="following-sibling::w:r[w:fldChar/@w:fldCharType='end'][1]"/>
              ...
              <xsl:variable name="wr_ref_content" select="$comPara/w:r[position()>number($wr_fldCharSeparate_Index) and number($wr_fldCharEnd_Index)>position()]"/>
              <xsl:value-of select="$wr_ref_content"/>


            </xsl:when>
            <xsl:when test="normalize-space($instrText)=''">

            </xsl:when>
            <xsl:otherwise>
              非引用的的w:insteText <xsl:value-of select="."/> 非引用的的w:insteText
            </xsl:otherwise>
          </xsl:choose>
        </xsl:when>
        <xsl:otherwise>
